<script setup>
</script>

<template>
    <div class="flex  lg:h-auto">
      <div class="w-5/12  text-headblue font-bold pt-11 pl-48 pr-24  bg-orangee">
        <p class="text-4xl uppercase">Lorem Ipsum is simply dummy text of the printing.</p>
    <div class="mt-6 mb-8">
        <button class="button bg-white text-headblue uppercase rounded-sm font-bold p-2 drop-shadow-md" style="letter-spacing: 0.25em;">hazte premium</button>
    </div>
    <p class="text-sm font-normal text-white">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
    </div>

      <div class="w-7/12">
        <img
          src="~/assets/thumbnails/contacto-surf.png"
          class=" image object-cover w-full"
        />
      </div>
        
      </div>
</template>

<style scoped>
.image {
  height:400px;
}
</style>